...
from app.helpers import delete_entry, get_entry_or_404, handle_installments_split, handle_entry_migration_or_update
...
def update_expense(
    ...
    installments: int | None = Query(None),
    session: Session = Depends(get_session)):
    return handle_entry_migration_or_update(
        session, ExpenseFixed, Expense, entry_id, entry, is_fixed, installments, "Expense not found"
    )


@router.delete("/{entry_id}", response_model=ExpenseRead)
def delete_expense(
    entry_id: UUID,
    is_fixed: bool = Query(False),
    session: Session = Depends(get_session)):
